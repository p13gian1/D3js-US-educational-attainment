...
var color = d3.scaleLinear().domain([0, 1/4*5000000, 2/4*5000000, 3/4*5000000, 5000000]).range(["#c10b7f", "#69c8f4", "#1eed4e", "#f3f700"]);
...
d3.json("assets/data/us.json", function(data) {
    ...
    $("foreignObject").mouseenter(function() {

     //$(.foreignobj.value).hide();
   responsiveVoice.speak(svg.foreignObject.foreignobj.name,'US English Male');
    ...
    function display(d) {
        ...
        document.forms[0].addEventListener("change", function() {
            ...
            .sum(function (d) {
                if (treeSumSortType == "number") {
                    color = d3.scaleLinear().domain([0, 1/4*5000000, 2/4*5000000, 3/4*5000000, 5000000]).range(["#c10b7f", "#69c8f4", "#1eed4e", "#f3f700"]);
                    return d["Total College"];
                } else if (treeSumSortType == "percent") {
                    color = d3.scaleLinear().domain([0, 1/4*50, 2/4*50, 3/4*50, 50]).range(["#c10b7f", "#69c8f4", "#1eed4e", "#f3f700"]);
                    return d["Percent College"];
